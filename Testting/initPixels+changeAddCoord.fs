module initPixels_changeAddCoord
...
let  rec findListCoordinates (segment:Segment) =
    let coordList = new List<Coordinate>()
    let rec addCoord = function
        |Pixel(coord,_) -> coordList.Add(coord)
        |Parent(seg1, seg2) ->  addCoord seg1 ; addCoord seg2 ;
    addCoord segment
    coordList
...
let createNeighboursFunction (pixelMap:Coordinate->Segment) (N:int) : (Segmentation -> Segment -> Set<Segment>) =
    ...
    let findListOfNeighbourCoordinates (segment:Segment) (N:int) =
        let coordList = segment |> findListCoordinates
        let maxValue = (pown 2 N )-1
        let neighbourCoordList = new List<Coordinate>()
        for ((x,y):Coordinate) in coordList do
            if ( ( x>= 0)  && (y>=0)  && (x<=maxValue) && (y<=maxValue)) then
                if ( x>0 ) then
                    neighbourCoordList.Add( (x - 1,y)) |> ignore
                if ( y>0 ) then
                    neighbourCoordList.Add( ( x,y-1)) |> ignore
                if ( x<maxValue ) then
                    neighbourCoordList.Add( ( (x + 1),y)) |> ignore
                if (y<maxValue) then
                   neighbourCoordList.Add( ( x,y+1)) |> ignore
    ...
    let neighbours (segmentation :Segmentation) (segment:Segment)  =
        ...
        let neighbourCoordList = findListOfNeighbourCoordinates segment N
        // Map the neighbours coords to the corresponding segment
        let listOfNeighbourSegment = new List<Segment>()
        for coord in neighbourCoordList do
            let currentSeg = pixelMap coord
            let rootSeg = findRoot segmentation currentSeg
            if (rootSeg <> segment) then
                listOfNeighbourSegment.Add(rootSeg) |>ignore

        Set.ofSeq listOfNeighbourSegment
