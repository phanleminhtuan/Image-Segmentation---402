...
let createPixelsArray (image:TiffModule.Image) size =
    let mutable currentPosition = 0
    let pixelsArray = Array.empty<Segment> (pown 2 (size+1)) 
    for i in 0 .. ( (pown 2 size) -1 ) do
        for j in 0 .. ( (pown 2 size) -1 ) do
            currentPosition <- 
